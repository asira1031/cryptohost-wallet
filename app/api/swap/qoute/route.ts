...
export async function POST(req: NextRequest) {
  try {
    ...
    const feeBps = process.env.NEXT_PUBLIC_SWAP_FEE_BPS?.trim() || "100";
    const feeWallet = process.env.NEXT_PUBLIC_SWAP_FEE_WALLET?.trim() || "";
    ...
    if (feeWallet) {
      params.set("fee", feeBps);
      params.set("referral", feeWallet);
    }
